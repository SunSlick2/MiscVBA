...
        ' New logic: Filter on multiple columns
        If ws.AutoFilterMode Then ws.AutoFilterMode = False
        rng.AutoFilter Field:=2, Criteria1:="Y"
       
        ' Create a union of the ranges to be filtered
        Dim UnionRng As Range
        Set UnionRng = Union(rng.Columns(13), rng.Columns(16), rng.Columns(22), rng.Columns(25), rng.Columns(28), rng.Columns(31))
        
        If Not Intersect(UnionRng, ws.AutoFilter.Range) Is Nothing Then
            ' Filter each column within the union range
            Dim fld As Long
            For Each cell In UnionRng.Columns
                fld = cell.Column
                ws.AutoFilter.Range.AutoFilter Field:=fld, Criteria1:="FX"
            Next cell
        End If
        
        ' Original data extraction logic remains the same
...
            ' Client Service Registry Report Handling (no changes)
